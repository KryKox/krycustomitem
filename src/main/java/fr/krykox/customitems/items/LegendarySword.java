...
/**
 * The type Legendary sword.
 */
public class LegendarySword extends ACustomItems {

    private final Ability ability = new Ability(AbilityType.XP, 3, 7);
    private static final String KEY_NAME = "legendary_sword";
    private final int maxDurability =  1000;
    ...
    public ItemStack getItem() {
        ...
        if (meta != null) {
            meta.setDisplayName("§6Épée légendaire");

            List<String> lore = new ArrayList<>();
            lore.add("§7Une épée forgée dans le feu des anciens.");
            lore.add("");
            ...
            meta.getEnchants().forEach((enchant, level) -> {
                ...
                String gradientName = GradientText.generateHexGradient(enchantName, startColor, endColor);
                lore.add("§b✦ " + gradientName + " §7Niveau " + level);
            });
    ...
    private String getCustomEnchantName(Enchantment enchant) {
        return switch (enchant.getKey().getKey()) {
            ...
            case "smite" -> "Châtiment";
            case "bane_of_arthropods" -> "Fléau des arthropodes";
            case "fire_aspect" -> "Aspect de feu";
    ...
    /**
     * Is legendary sword boolean.
     *
     * @param item the item
     * @return the boolean
     */
    public static boolean isLegendarySword(ItemStack item) {
